...
#define DATA_INTERVAL (10000U)
#define X_BLOCK_TIME  (1000U) 
...
static esp_err_t sht31_read_temp_humi()
{
    printf("usao u zadatak");
    ...
    float temp = -45 + (175 *(float)(data_rd[0] * 256 + data_rd[1])/ 65535.0);
    float humi = 100 * (float)(data_rd[3] * 256 + data_rd[4]) / 65535.0;

    send_temp_hum_data(temp, humi);
